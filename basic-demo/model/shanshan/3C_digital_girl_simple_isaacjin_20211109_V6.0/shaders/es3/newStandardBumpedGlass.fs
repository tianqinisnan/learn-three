...
void main()
{
    ...
    {
        n = normalize(texture(bumptex, v_UV4).xyz *  2.0 - 1.0);
        n.xy *= bumpscale;
    ...
    float env_sat = dot(env,vec3(0.2125, 0.7154,0.0721));
    float spec_sat = dot(spec,vec3(0.2125, 0.7154,0.0721));
    out_fragColor.a = min(env_sat+spec_sat+albedoColor.a,1.0);
}
