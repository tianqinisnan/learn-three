...
void main()
{
	...
	if(bool(albedousage)){
		albedoColor = texture2D(albedo,v_UV1);
	}
	...
	if(bool(speculartexusage)){
		_spec1 = texture2D(speculartex,_spec_UV);
	}
	vec4 transmask = vec4(0.0);
	if(bool(transmasktexusage)){
		transmask = texture2D(transmasktex,v_UV1)*transmission;
	}
	...
	if(bool(shadowtexusage)){
		...
		shadow = texture2D(shadowtex,uv_shadow);
	}
    ...
    gl_FragColor = (maincol + _spec + _spec1 + fresnel)*shadow;
