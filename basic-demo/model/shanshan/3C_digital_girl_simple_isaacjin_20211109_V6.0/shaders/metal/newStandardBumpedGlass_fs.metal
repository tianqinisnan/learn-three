...
fragment xlatMtlShaderOutput newStandardBumpedGlass_fs (xlatMtlShaderInput _mtl_i [[stage_in]], constant xlatMtlShaderUniform& _mtl_u [[buffer(1)]]
  ...
  ,   texture2d<float> emissivetex [[texture(7)]], sampler _mtlsmp_emissivetex [[sampler(7)]])
{
  ...
  float tmpvar_46;
  tmpvar_46 = min (((
    dot (env_4, (float3)float3(0.2125, 0.7154, 0.0721))
   +
    dot (tmpvar_31, (float3)float3(0.2125, 0.7154, 0.0721))
  ) + albedoColor_7.w), (float)1.0);
  _mtl_o.gl_FragColor.w = float(tmpvar_46);
  return _mtl_o;
}
