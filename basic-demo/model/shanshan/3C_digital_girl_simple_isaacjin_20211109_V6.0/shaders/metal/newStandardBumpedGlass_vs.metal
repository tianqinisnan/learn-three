...
vertex xlatMtlShaderOutput newStandardBumpedGlass_vs(xlatMtlShaderInput _mtl_i [[stage_in]], constant xlatMtlShaderUniform& _mtl_u [[buffer(1)]])
{
  ...
  _mtl_o.v_UV4 = ((_mtl_i.inTexCoord0 * _mtl_u.bumptex_uv.xy) + _mtl_u.bumptex_uv.zw);
  float4 tmpvar_2;
  tmpvar_2.w = 1.0;
  ...
  float4 tmpvar_4;
  tmpvar_4.w = 0.0;
  tmpvar_4.xyz = _mtl_i.inVertexTangent;
  _mtl_o.v_wT = normalize((_mtl_u._MIT * tmpvar_4).xyz);
  float4 tmpvar_5;
  tmpvar_5.w = 0.0;
  tmpvar_5.xyz = _mtl_i.inVertexBinormal;
  _mtl_o.v_wB = normalize((_mtl_u._MIT * tmpvar_5).xyz);
